...
| 규칙 | 좋은 예 | 나쁜 예 |
| --- | --- | --- |
| 명사 사용 | `GET /api/projects` | `GET /api/getProjects` |
| 복수형 사용 | `/api/users`, `/api/applications` | `/api/user`, `/api/application` |
| 계층 구조 | `/api/projects/1/applications` | `/api/getProjectApplications` |
| 소문자+하이픈 | `/api/board-posts` | `/api/boardPosts` |
...
| 메서드 | 용도 | 멱등성 | 예시 |
| --- | --- | --- | --- |
| `GET` | 리소스 조회 | ⭕ | `GET /api/projects` |
| `POST` | 리소스 생성 | ❌ | `POST /api/auth/signup` |
| `PUT` | 리소스 전체 수정 | ⭕ | `PUT /api/users/me` |
| `PATCH` | 리소스 부분 수정 | ❌ | `PATCH /api/applications/1/status` |
| `DELETE` | 리소스 삭제 | ⭕ | `DELETE /api/projects/1` |
...
| 역할 | 접근 가능 API | 설명 |
| --- | --- | --- |
| **공개 (미인증)** | `GET /api/projects`, `GET /api/projects/:id`, `/api/auth/**` | 모집글 조회만 허용 |
| **USER** | 공개 + 모집글 생성, 지원 신청, 내 정보 관리 | 로그인 회원 |
| **MEMBER** | USER + 프로젝트 내부 게시판 접근 | 특정 프로젝트 합류 팀원 |
| **OWNER** | MEMBER + 지원자 승인/거절, 마감, 멤버 관리 | 프로젝트 개설자 |
| **ADMIN** | 전체 + 관리자 전용 | 악성 유저 제재, 부적절한 글 삭제 |
...
- **닉네임 (서버 자동 생성)**: 별도 입력받지 않음. 서버에서 이메일의 `@` 앞부분을 추출하여 초기 닉네임으로 할당함. (예: `user@mate.com` -> `user`)
- **기술 스택**: 최소 1개 이상 등록 필수. 미등록 시 프로젝트 지원 불가.
- **휴대폰 번호:** 휴대폰 번호는 중복될 수 없으며, 아이디/비밀번호 찾기의 본인 확인 수단으로 사용됨.
...
```
/api/projects?category=PROJECT&status=RECRUITING&stacks=Java,Spring&sort=latest&page=0&size=10
```
...
```json
GET /api/projects/{projectId}/posts?page=0&size=10
Authorization: Bearer {JWT_TOKEN}
```
...
| --- | --- | --- | --- | --- |
| **인증** | **POST** | `/api/auth/signup` | 회원가입 (전화번호 포함) | GUEST |
|  | **POST** | `/api/auth/login` | 로그인 (JWT 발급) | GUEST |
|  | **POST** | `/api/auth/refresh` | 토큰 재발급 (Access 만료 시) | GUEST |
|  | **GET** | `/api/auth/find-id` | **아이디 찾기 (전화번호 기준)** | **GUEST** |
|  | **POST** | `/api/auth/find-password` | **비밀번호 찾기 (임시 비번 발급)** | **GUEST** |
| **마이페이지** | **GET** | `/api/users/me` | 내 프로필 정보 조회 (전화번호 포함) | USER |
|  | **PUT** | `/api/users/me` | 내 프로필 정보 수정 (전화번호 포함) | USER |
|  | **GET** | `/api/users/me/activities` | 내 활동 종합 조회 (개설글 + 지원내역) | USER |
|  | **DELETE** | `/api/users/me` | 회원 탈퇴 (Soft Delete) | USER |
| **모집글** | GET | `/api/projects` | 모집글 목록 조회 (필터/검색) | GUEST |
|  | GET | `/api/projects/{id}` | 모집글 상세 조회 | GUEST |
|  | POST | `/api/projects` | 모집글 등록 | USER |
|  | PUT | `/api/projects/{id}` | 모집글 수정 (인원 제한 체크) | OWNER |
|  | PATCH | `/api/projects/{id}/close` | 모집 수동 마감 | OWNER |
|  | DELETE | `/api/projects/{id}` | 모집글 삭제 (Soft Delete) | OWNER |
| **지원/매칭** | POST | `/api/projects/{id}/applications` | 프로젝트 참여 지원 | USER |
|  | DELETE | `/api/applications/{id}` | 지원 취소 (PENDING 상태만 가능) | USER |
|  | PATCH | `/api/applications/{id}/status` | 지원 승인/거절 (인원 반영) | OWNER |
| **팀 소통** | GET | `/api/projects/{id}/posts` | 내부 게시판 목록 조회 | MEMBER, OWNER |
|  | POST | `/api/projects/{id}/posts` | 내부 게시글 작성 | MEMBER, OWNER |
|  | PUT | `/api/posts/{id}` | 내부 게시글 수정 | 작성자 |
|  | DELETE | `/api/posts/{id}` | 내부 게시글 삭제 (Soft Delete) | 작성자, OWNER |
|  | POST | `/api/posts/{id}/comments` | 댓글 작성 | MEMBER, OWNER |
|  | PATCH | `/api/comments/{id}` | 댓글 수정 | 작성자 |
|  | DELETE | `/api/comments/{id}` | 댓글 삭제 | 작성자, OWNER |
...
- [V] **RESTful 원칙 준수**: 리소스 중심의 URL 설계 및 계층 구조 적용
- [V] **일관된 네이밍**: 모든 엔드포인트에 소문자, 하이픈(-), 복수형 명사 사용
- [V] **HTTP 메서드 최적화**: GET(조회), POST(등록), PATCH(수정), DELETE(삭제)의 올바른 용도 사용
- [V] **표준 응답 포맷**: 모든 응답이 `{ "success", "data", "message" }` 공통 규격을 따르는가?
- [V] **활동 통합 조회**: 마이페이지 탭 구조 대응을 위해 `activities` API가 효율적으로 설계되었는가?

### **6.2 보안 및 검증**

- [V] **입력값 검증(Validation)**: 닉네임, 비밀번호, **전화번호(숫자 11자)** 등 유효성 검사 적용
- [V] **개인정보 보안**: 아이디 찾기 응답 시 이메일 주소의 일부를 **백엔드에서 마스킹 처리()**하여 반환하는가?
- [V] **비밀번호 복구**: 비밀번호 분실 시 **임시 비밀번호 생성 및 화면 노출 프로세스**가 설계되었는가?
- [V] **권한 제어(RBAC)**: 역할별(GUEST, USER 등) API 접근 권한이 명확히 구분되었는가?

### **6.3 성능 및 데이터 정합성**

- [V] **데이터 정합성(트랜잭션)**: 임시 비밀번호 생성 시 **[DB 업데이트]와 [결과 반환]이 하나의 프로세스**로 설계되었는가?
- [V] **인덱스 설계**: 조회가 빈번한 `email`, `nickname`, **`phone_number`** 컬럼에 인덱스 적용 여부